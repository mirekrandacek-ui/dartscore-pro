...
old = "    const L = (cs, en) => lang === 'cs' ? cs : en;"
new = r'''    const STATS_I18N = {
      cs: {
...
if old not in text:
    raise SystemExit('Statistics language helper anchor not found')
...
"<strong style={{ fontSize:18 }}>📊 {L('Statistiky','Statistics')}</strong>": "<strong style={{ fontSize:18 }}>📊 {S('statistics')}</strong>",
"{L('Přehled','Overview')}": "{S('overview')}",
">Head to Head</button>": ">{S('h2h')}</button>",
...
"{metricCard(fmt(overview.avg3), '3-dart AVG')}": "{metricCard(fmt(overview.avg3), S('avg3'))}",
"{metricCard(fmt(overview.first9Avg), 'First 9')}": "{metricCard(fmt(overview.first9Avg), S('first9'))}",
...
"<strong>{L('Scoring','Scoring')}</strong>": "<strong>{S('scoring')}</strong>",
"<strong>{L('Výkony','Performance')}</strong>": "<strong>{S('performance')}</strong>",
"L('Nejlepší leg (šipky)','Best leg (darts)')": "S('bestLeg')",
"L('Průměr šipek / leg','Avg darts / leg')": "S('avgDartsLeg')",
"L('Nejvyšší nához','Highest score')": "S('highestScore')",
"L('Win rate','Win rate')": "S('winRate')",
"{L(`Detailní metriky jsou dostupné u ${overview.detailedMatches} z ${overview.matches} zápasů. Starší zápasy zůstávají započítané do výher.`,`Detailed metrics are available for ${overview.detailedMatches} of ${overview.matches} matches. Older matches still count toward wins.`)}":
"{S('detailed', { detailed: overview.detailedMatches, total: overview.matches })}",
"{h2hGames.length} {L('vzájemných zápasů','matches')}": "{h2hGames.length} {S('matches')}",
"<span style={{ opacity:.55 }}>Win rate</span>": "<span style={{ opacity:.55 }}>{S('winRate')}</span>",
"{compareRow(h1.legsWon, 'Legs', h2.legsWon)}": "{compareRow(h1.legsWon, S('legs'), h2.legsWon)}",
"{compareRow(h1.setsWon, 'Sets', h2.setsWon)}": "{compareRow(h1.setsWon, S('sets'), h2.setsWon)}",
"{compareRow(fmt(h1.avg3), '3-dart AVG', fmt(h2.avg3))}": "{compareRow(fmt(h1.avg3), S('avg3'), fmt(h2.avg3))}",
"{compareRow(fmt(h1.first9Avg), 'First 9', fmt(h2.first9Avg))}": "{compareRow(fmt(h1.first9Avg), S('first9'), fmt(h2.first9Avg))}",
"{compareRow(pct(h1.checkoutPct), 'Checkout %', pct(h2.checkoutPct))}": "{compareRow(pct(h1.checkoutPct), S('checkoutPct'), pct(h2.checkoutPct))}",
...
"<strong>{L('Forma posledních 5','Last 5 form')}</strong>": "<strong>{S('last5')}</strong>",
"<strong>{L('Trend AVG','AVG trend')}</strong>": "<strong>{S('avgTrend')}</strong>",
"{L('Starší zápasy jsou započítané do H2H skóre a win rate. Detailní metriky se plní jen u nově uložených zápasů.','Older matches count toward H2H score and win rate. Detailed metrics populate for newly saved matches.')}": "{S('older')}",
"<strong>{L('Historie vzájemných zápasů','Match history')}</strong>": "<strong>{S('matchHistory')}</strong>",
"{L('vyhrál','won')}": "{S('won')}",
"{compareRow(fmt(s1?.first9Avg), 'First 9', fmt(s2?.first9Avg))}": "{compareRow(fmt(s1?.first9Avg), S('first9'), fmt(s2?.first9Avg))}",
"{compareRow(s1?.legsWon ?? '—', 'Legs', s2?.legsWon ?? '—')}": "{compareRow(s1?.legsWon ?? '—', S('legs'), s2?.legsWon ?? '—')}",
"{compareRow(s1?.setsWon ?? '—', 'Sets', s2?.setsWon ?? '—')}": "{compareRow(s1?.setsWon ?? '—', S('sets'), s2?.setsWon ?? '—')}",
"{compareRow(s1?.highestCheckout ?? '—', L('Checkout','Checkout'), s2?.highestCheckout ?? '—')}": "{compareRow(s1?.highestCheckout ?? '—', S('checkout'), s2?.highestCheckout ?? '—')}",
}

for before, after in replacements.items():
    if before not in text:
        raise SystemExit(f'Missing expected statistics string: {before[:90]}')
    text = text.replace(before, after, 1)

# These labels are intentionally universal darts notation and stay unchanged: 60+, 100+, 140+, 180, 180s, AVG, vs.
path.write_text(text, encoding='utf-8')
print('Statistics translations applied for cs, en, de, es, nl, ru, zh.')
